...
```{r }
sesame <- read.csv(here::here("ROS-Examples-master/Sesame/data/sesame.csv"))
head(sesame)
```
...
IV_brm_a <- brm(f1 + f2, data=sesame, seed=SEED, backend = "cmdstanr", cores = 4)
...
IV_brm_b <- brm(f1 + f2, data=sesame, seed=SEED,  backend = "cmdstanr", cores = 4)
```
```{r }
